...
def save_order_to_db(pedido_final: List[dict]):
    """Salva o pedido consolidado no banco de dados 'pedidos.db' usando named parameters
    para garantir que cada valor vá para a coluna correta (evita desalinhamento)."""
    conn = None
    try:
        ...
        # Prepara a lista de colunas (nomeadas)
        loja_cols_names = [f"loja_{loja}" for loja in LISTA_LOJAS]

        columns = [
            "codigo", "produto", "ean", "embalagem", "data_pedido",
            "usuario_pedido", "status_item"
        ] + loja_cols_names + ["total_cx", "status_aprovacao"]

        placeholders = ", ".join([f":{col}" for col in columns])

        query = f"INSERT INTO pedidos_consolidados ({', '.join(columns)}) VALUES ({placeholders})"

        for item in pedido_final:
            # --- Preparação e sanitização dos valores ---
            raw_codigo = item.get('Codigo', None)
            codigo_val = None
            try:
                # tenta converter direto se for numérico
                if isinstance(raw_codigo, (int, float)):
                    codigo_val = int(raw_codigo)
                else:
                    s = str(raw_codigo)
                    # extrai a maior sequência de dígitos (evita strings com vírgulas/artefatos)
                    import re
                    m = re.search(r'(\d+)', s)
                    if m:
                        codigo_val = int(m.group(1))
            except Exception:
                codigo_val = None

            data_dict = {
                "codigo": codigo_val,
                "produto": item.get('Produto', None),
                "ean": str(item.get('EAN', '')) if item.get('EAN', None) is not None else None,
                "embalagem": int(item.get('Embalagem')) if item.get('Embalagem') not in (None, '') else None,
                "data_pedido": data_pedido_str,
                "usuario_pedido": usuario,
                "status_item": item.get('Status', None),
                "total_cx": int(item.get('Total_CX', 0)),
                "status_aprovacao": 'Pendente'
            }

            # Adiciona valores das lojas (garantindo inteiros)
            for loja in LISTA_LOJAS:
                chave = f"loja_{loja}"
                valor = item.get(chave, 0)
                try:
                    data_dict[chave] = int(valor) if valor is not None else 0
                except Exception:
                    data_dict[chave] = 0

            # Executa com named parameters (garante mapeamento por nome)
...
def show_pedidos_page():
    ...
    if not st.session_state['logged_in']:
        # --- Layout reduzido e centralizado ---
        st.markdown("""
            <style>
            .login-box {
                max-width: 400px;
        ...
        st.markdown('<div class="login-title">🔒 Sistema de Gestão de Produtos</div>', unsafe_allow_html=True)
        st.markdown('<div class="login-sub">Área de Login</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="footer-note">Para novos usuários ou problemas de senha, contate o administrador do sistema.</div>', unsafe_allow_html=True)
