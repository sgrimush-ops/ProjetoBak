...
def load_hist_data(file_path: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # Garante que a situação seja string limpa para comparação
...
def show_historico_page(engine, base_data_path):
    ...
    if codigo_para_filtrar:
        ...
        if df_item_hist.empty:
            ...
            # Agora usamos .isin() que aceita múltiplos valores e retorna booleano
            condicao_nao_atendido = df_item_hist['Situacao'].isin(['1', '7', 1, 7]) # Aceita string ou int por segurança
        ...
        # Lógica corrigida para Status 1 ou 7 usando .isin()
        condicao_nao_atendido_total = df_hist_mensal['Situacao'].isin(['1', '7', 1, 7])
    ...
    if not df_final_grafico.empty:
        ...
        # MUDANÇA: Formata Eixo X como string DD/MM para remover horas do gráfico
